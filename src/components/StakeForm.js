            ...
            <div>
                ...
                <div className="w-full flex items-center justify-between">
                    ...
                    {(isLoading && !Withdraw) ? (
                        ...
                        <>
                            <button className={classNames('bg-white text-black uppercase px-4 py-2 ml-4 text-sm font-bold transition ease-in-out duration-300 hover:bg-[#b28760] hover:text-white min-w-[125px]', { 'hidden': approve })} onClick={handleApprove}>Approve</button>

                            <button className={classNames('bg-white text-black uppercase px-4 py-2 ml-4 text-sm font-bold transition ease-in-out duration-300 hover:bg-[#b28760] hover:text-white min-w-[125px]', { 'hidden': !approve })} onClick={handleStake}>Stake</button>
                        </>
                    )}
            ...
            <div className="mt-2">
                ...
                <div className="w-full flex items-center justify-between">
                    <div className='w-3/4 relative'>
                        ...
                        <span className='absolute top-0 right-0 mr-3 mt-1 cursor-pointer hover:text-yellow-500' onClick={() => setBalance((plan.plan == 0) ? blockChainData.StakeBalance.plan0 : (plan.plan == 1) ? blockChainData.StakeBalance.plan1 : (plan.plan == 2) ? blockChainData.StakeBalance.plan2 : (plan.plan == 3) ? blockChainData.StakeBalance.plan3 : (plan.plan == 4) ? blockChainData.StakeBalance.plan4 : 0)}>Max</span>
                    </div>

                    {(isLoading && Withdraw) ? (
                        <LoadingSpinner />
                    ) : (
                        <button className="bg-white text-black uppercase px-4 py-2 ml-4 text-sm font-bold transition ease-in-out duration-300 hover:bg-[#b28760] hover:text-white min-w-[125px]" onClick={handleWithdraw}>Withdraw</button>
                    )}
                </div>
